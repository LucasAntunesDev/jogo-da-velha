...
const Jogo = () => {
  ...
  const [casas, setCasas] = useState(() => {
    const savedCasas = localStorage.getItem('casas');
    if (savedCasas) {
      try {
        return JSON.parse(savedCasas);
      } catch (error) {
        console.error('Erro ao analisar casas do localStorage:', error);
        return Array(9).fill(null); // Retorna um array padrão em caso de erro
      }
    }
  ...
  const [dificuldade, setDificuldade] = useState(localStorage.getItem('dificuldade') || 'fácil');
  ...
  const bloquear = (verificador, novasCasas) => {
    for (let combinacao of combinacoesVitoria) {
      ...
      if (
        (primeiro === verificador && segundo === verificador && terceiro === null) ||
        (primeiro === verificador && terceiro === verificador && segundo === null) ||
        (segundo === verificador && terceiro === verificador && primeiro === null)
      ) {
  ...
  const handleClick = (index) => {
    ...
    if (novasCasas[index] === null && !resultado && !desativado) {
      if (jogadorAtual === 0) {
        ...
        setTimeout(() => {
          const indexMaquina = dificuldade === 'fácil'
